...
import { getQuizMetrics, createQuizMetrics, updateQuizMetrics, deleteQuizMetrics } from '../services/api';
...
export default function QuizSE() {
  ...
  const totals = metrics.reduce((acc, m) => ({
    ...
    quiz_inicio: acc.quiz_inicio + m.quiz_inicio,
    quiz_end: acc.quiz_end + m.quiz_end,
  ...
  const avgTaxaConclusao = totals.quiz_inicio > 0 ? (totals.quiz_end / totals.quiz_inicio * 100) : 0;
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-800">Quiz SE - Funil de Leads</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        <KPICard title="Início Quiz" value={totals.quiz_inicio.toLocaleString()} color="orange" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <KPICard title="Conclusão Quiz" value={totals.quiz_end.toLocaleString()} color="teal" />
        <KPICard title="Taxa Conclusão" value={`${avgTaxaConclusao.toFixed(2)}%`} color="red" />
        <KPICard title="Leads" value={totals.leads.toLocaleString()} color="green" />
        <KPICard title="CPL Médio" value={`R$ ${avgCPL.toFixed(2)}`} color="blue" />
      </div>
              ...
              {loading ? (
                ...
                metrics.map((metric) => (
                  <tr key={metric.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900 text-right">{metric.ctr.toFixed(2)}%</td>
