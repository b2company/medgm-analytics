...
function QuizSETab() {
  ...
  const [dataImportacao, setDataImportacao] = useState(new Date().toISOString().split('T')[0]);
  ...
  const handleImportarMeta = async () => {
    try {
      setLoadingCampanhas(true);
      const campanhas = await getMetaCampanhasParaImportar(dataImportacao, dataImportacao);
      setCampanhasDisponiveis(campanhas);
      setShowImportModal(true);
    } catch (error) {
      console.error('Erro ao buscar campanhas:', error);
      alert('Erro ao buscar campanhas do Meta Ads. Verifique se está conectado.');
    } finally {
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-800 mb-2">Nenhuma métrica cadastrada</h3>
          <p className="text-gray-600 mb-6">
            Comece adicionando sua primeira métrica de Quiz SE para acompanhar o desempenho das suas campanhas.
          </p>
          <button
            onClick={() => setShowModal(true)}
        ...
        <>
          ...
          {/* Modal de Importação do Meta Ads */}
          {showImportModal && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
              <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6">
                  <div className="flex justify-between items-center mb-6">
                    <div>
                      <h2 className="text-2xl font-bold text-gray-800">Importar do Meta Ads</h2>
                      <p className="text-gray-600 mt-1">Selecione uma campanha para importar as métricas</p>
                    </div>
                  ...
                  <div className="mb-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                    <p className="text-sm text-blue-800">
                      📅 Data selecionada: <strong>{new Date(dataImportacao + 'T00:00:00').toLocaleDateString('pt-BR')}</strong>
                    </p>
                  </div>

                  {campanhasDisponiveis.length === 0 ? (
                    <div className="text-center py-12 text-gray-500">
                      Nenhuma campanha encontrada para esta data.
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {campanhasDisponiveis.map((campanha) => (
                        <div
                          ...
                          onClick={() => handleSelecionarCampanha(campanha)}
                        >
                          <div className="flex justify-between items-start mb-3">
                            <div className="flex-1">
                              <h3 className="font-semibold text-gray-900">{campanha.name}</h3>
                              <p className="text-sm text-gray-500 mt-1">ID: {campanha.id}</p>
                            </div>
                            <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                              campanha.status === 'ACTIVE' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                            }`}>
                              {campanha.status}
                            </span>
                          </div>
                          <div className="grid grid-cols-3 gap-4 text-sm">
                            <div>
                              ...
                              <p className="font-medium text-gray-900">R$ {campanha.verba.toFixed(2)}</p>
                            </div>
                            <div>
                              <span className="text-gray-500">Impressões:</span>
                              <p className="font-medium text-gray-900">{campanha.impressoes.toLocaleString()}</p>
                            </div>
                            <div>
                              <span className="text-gray-500">Cliques:</span>
                              <p className="font-medium text-gray-900">{campanha.cliques.toLocaleString()}</p>
                            </div>
                          </div>
                        </div>
                      ))}
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          <h3 className="text-2xl font-bold text-gray-800 mb-2">Nenhuma métrica cadastrada</h3>
          <p className="text-gray-600 mb-6">
            Comece adicionando sua primeira métrica de Venda Direta para acompanhar vendas e ROAS das suas campanhas.
          </p>
          <button
            onClick={() => setShowModal(true)}
            className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 transition inline-flex items-center gap-2"
